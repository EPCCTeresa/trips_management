...
class SegmentManagement
  ...
  def sort_by_destination
    ...
    @entries.each_with_index do |entry, i|
      ...
      if result[new_destination].nil?	# Is a new group
        result[new_destination] = []
      end
      result[new_destination].push(entry)
    end
    @result = result
  end
  ...
  def order_segments(segments)
    pos = []
    i = 1
    segments.each_with_index{ |str,i| pos << i if str.include?("Hotel") } # Get positions of segments that contains stays(We just consider "Hotel" as stay but it can be extended) 
  ...
  def compute_destination(entry, prev_entry = "", next_entry = "", i, result)
    ...
    destination = new_destination if new_destination != destination
  ...
  def is_connection_segment?(entry1, entry2) 
    ...
    return false if entry1.start_with?("Hotel") || entry2.start_with?("Hotel")
